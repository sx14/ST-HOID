...
if __name__ == '__main__':
    ...
    for pid_vid in sorted(all_trajs):
        ...
        vid_frm_dir = os.path.join(image_root, pkg_id, vid_id)
        vid_frm_list = sorted(os.listdir(vid_frm_dir))
        vid_frm = cv2.imread(os.path.join(vid_frm_dir, vid_frm_list[0]))
        num_frames = len(vid_frm_list)
        ...
        for frm_idx in range(len(fid2dets)):
            ...
            for box_ind in range(len(tids)):
                if is_human(cates[box_ind]):
                    kps = fid2dets[frm_idx][box_ind]['kps']
                    if kps is not None:
            ...
            # Load the demo image
            im_file = os.path.join(vid_frm_dir, vid_frm_list[frm_idx])
